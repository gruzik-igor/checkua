...
class wl_analytic_model {
	...
	{
		$views = $this->db->getQuery("SELECT SUM(`unique`) as totalUsers, SUM(`cookie`) as newUsers, SUM(`views`) as viewsCount FROM `wl_statistic_views`");
		...
		$views->tableData = $this->db->getQuery("SELECT * FROM `wl_statistic_views`", 'array');
