                        ...
                        <fieldset>
                            <div class="row">
                                ...
                                <div class="col-md-12 text-center">
                                	<button class="btn btn-lg btn-success" onclick="setUser(0, <?= $_SESSION['option']->newUserType?>)">Гостьовий режим</button>
                                </div>
                            </div>
						</fieldset>
					</div>
				...
				success:function (res) {
					if(res['result'] == true){
						...
						$.each(res['user'], function (index,value) {
							$("#userTable>table>tbody").append("<tr><td>"+value.id+"</td><td>"+value.name+"</td><td>"+value.email+"</td><td><button onclick='setUser("+value.id+", "+value.type+")' class='btn btn-xs btn-success'><i class='fa fa-check'></i></button></td></tr>");
						})
					} else {
						$("#userTable").hide();
	...
	function saveNewUser() {
		...
		if(name && password && (email || phone))
		{
			$.ajax({
				...
				success:function (res) {
					if(res['result'] == true){
						setUser(res['id'], <?= $_SESSION['option']->newUserType?>);
					} else {
