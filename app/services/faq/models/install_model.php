...
class install{
	...
	public function alias($alias = 0, $table = '')
	{
		...
		$query = "CREATE TABLE IF NOT EXISTS `{$this->table_service}{$table}_questions` (
		...
		if($this->options['useGroups'] > 0){
			$query = "CREATE TABLE IF NOT EXISTS `{$this->table_service}{$table}_groups` (
						  `id` int(11) NOT NULL AUTO_INCREMENT,
						  `alias` text,
						  `position` int(11) NOT NULL,
						  `active` tinyint(1) NOT NULL,
	...
	{
		$this->db->executeQuery("DROP TABLE IF EXISTS {$this->table_service}{$table}_questions");
		$this->db->executeQuery("DROP TABLE IF EXISTS {$this->table_service}{$table}_groups");
