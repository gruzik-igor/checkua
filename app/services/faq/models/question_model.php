...
class question_model {
	...
	{
		$this->db->select($this->table().' as q', '*', $alias, 'alias');
		$this->db->join('wl_users as a', 'name as author_add_name', '#q.author_add');
		$this->db->join('wl_users as e', 'name as author_edit_name', '#q.author_edit');
		$where_join['alias'] = $_SESSION['alias']->id;
		$where_join['content'] = '#q.id';
	...
	public function add()
	{
		$data = array();
		$group = 0;
		if($this->data->post('group')) $group = $this->data->post('group');
		$data['group'] = $group;
		$data['active'] = 1;
		...
		if($this->db->insertRow($this->table(), $data))
		{
			...
			$data = array('alias' => '');
			$data['position'] = $this->db->getCount($this->table(), $group, 'group');
		...
		if($go){
			...
			if($_SESSION['option']->useGroups){
				...
				if($question->group != $faq['group']){
					$this->db->executeQuery("UPDATE `{$this->table()}` SET `position` = position - 1 WHERE `position` > '{$question->position}' AND `group` = '{$question->group}'");
			...
			if($this->db->updateRow($this->table(), $faq, $id)) {
				if($_SESSION['option']->useGroups){
					$group = $this->db->getAllDataById($this->table('_groups'), $faq['group']);
					if($group){
	...
	public function delete($id)
	{
		...
		if($question){
			...
			$this->db->executeQuery("UPDATE `{$this->table()}` SET `position` = position - 1 WHERE `position` > '{$question->position}' AND `group` = '{$question->group}'");
			$this->db->executeQuery("DELETE FROM wl_ntkd WHERE alias = '{$_SESSION['alias']->id}' AND content = '{$question->id}'");
			if($question->group > 0 && $_SESSION['option']->useGroups){
				$group = $this->db->getAllDataById($this->table('_groups'), $question->group);
				return '/'.$group->alias;
			}
			return '';
		}
