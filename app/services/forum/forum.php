...
class forum extends Controller {
    ...
    public function index($uri)
    {
		...
		if(is_numeric($id))
		{
			...
			if($article && ($article->active == 1 || $this->userCan())){
				...
				$otherArticlesByGroup = $this->forum_model->getArticles($article->group);
		...
		elseif($id != '' && count($url) > 1)
		{
			if($_SESSION['option']->useGroups)
			{
				...
				if($group && ($group->active == 1 || $this->userCan($_SESSION['alias']))){
					...
					$articles = $this->forum_model->getArticles($group->id);
		...
		{
			$articles = $this->forum_model->getArticles();
			if($_SESSION['option']->useGroups)
			{
	...
	public function search()
	{
		if(isset($_GET['name']) || isset($_GET['group'])){
			...
			if(isset($_GET['group'])){
				...
				$group = $this->db->getQuery("SELECT g.*, n.name, n.title FROM `{$this->forum_model->table('_groups')}` as g LEFT JOIN `wl_ntkd` as n ON n.alias = {$_SESSION['alias']->id} AND n.content = -g.id {$language} WHERE g.link = '{$this->data->get('group')}'");
				if($group) {
			...
			$articles = $this->forum_model->getArticles($group);
    ...
    {
    	$this->load->smodel('forum_search_model');
    	return $this->forum_search_model->getByContent($content);
    }
	...
	public function __get_Articles($data = array())
	{
		...
		$this->load->smodel('forum_model');
		return $this->forum_model->getArticles($group);
	}

	public function __get_Groups()
	{
		$this->load->smodel('forum_model');
		return $this->forum_model->getGroups();
	}
