...
class groups_model {
	...
	public function delete($id)
	{
		...
		if($group)
		{
			...
			else
			{
				...
				$this->db->executeQuery("UPDATE `{$this->table('_articles')}` SET `group` = '{$group->parent}' WHERE `group` = '{$group->id}'");
			}
	...
	{
		$articles = $this->db->getAllDataByFieldInArray($this->table('_articles'), $group, 'group');
