    ...
    <table class="table table-striped"><tbody class="files">
        <?php foreach($photos as $p){ ?>
            <tr id="photo-<?=$p->id?>" class="template-download fade in">
                <td class="preview">
                    <a href="<?=IMG_PATH.$_SESSION['alias']->alias?>/<?=$article->id.'/'.$p->name?>" data-gallery>
                        <img src="<?=IMG_PATH.$_SESSION['alias']->alias?>/<?=$article->id.'/s_'.$p->name?>">
                    </a>
                </td>
                <td>
...
$_SESSION['alias']->js_load[] = APP_PATH.'services/'.$_SESSION['service']->name.'/views/admin/articles/edit_tabs/tab-photo.js';
?>
