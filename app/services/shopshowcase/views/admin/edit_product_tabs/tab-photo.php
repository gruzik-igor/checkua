    ...
    <table class="table table-striped"><tbody class="files">
        <?php foreach($photos as $p){ ?>
            <tr id="photo-<?=$p->id?>" class="template-download fade in">
                <td class="preview">
                    <a href="<?=IMG_PATH.$_SESSION['alias']->alias?>/<?=$product->id.'/'.$p->name?>" data-gallery>
                        <img src="<?=IMG_PATH.$_SESSION['alias']->alias?>/<?=$product->id.'/s_'.$p->name?>">
                    </a>
                </td>
                <td>
                ...
                <td class="navigation">
                    ...
                    <br>
                    <span id="pea-saveing-<?=$p->id?>" class="saveing"><img src="<?=SITE_URL?>style/images/icon-loading.gif">Зберігання..</span>
                </td>
            </tr>
        <?php } ?>
    </tbody></table>
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr id="photo-{%=file.id%}" class="template-download fade">
        ...
        <td class="navigation">
            ...
            <br>
            <span id="pea-saveing-{%=file.id%}" class="saveing"><img src="<?=SITE_URL?>style/images/icon-loading.gif">Зберігання..</span>
        </td>
    </tr>
{% } %}
</script>
...
$(function () {
    ...
    $('#fileupload').fileupload({
        url: '<?=SITE_URL.'admin/'.$_SESSION['alias']->alias?>/photo_add/<?=$product->id?>',
        autoUpload: true,
...
<script type="text/javascript">
    function savePhoto(id, e){
        ...
        $.ajax({
            url: "<?=SITE_URL.'admin/'.$_SESSION['alias']->alias?>/photo_save",
            type: 'POST',
            data: {
                photo: id,
                name: e.name,
	...
	function deletePhoto(id){
		if (confirm("Ви впевнені, що хочете видалити фотографію? \nУВАГА, інформація відновленню НЕ ПІДЛЯГАЄ!")) {
			$('#pea-saveing-'+id).css("display", "block");
			$.ajax({
				url: "<?=SITE_URL.'admin/'.$_SESSION['alias']->alias?>/photo_delete",
				type: 'POST',
				data: {
					photo: id,
					json: true
...
<!-- Bootstrap styles -->
<link rel="stylesheet" href="<?=SITE_URL?>assets/bootstrap/bootstrap-wl.css">
