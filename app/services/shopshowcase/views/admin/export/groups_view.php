        ...
        <div class="panel panel-inverse">
			...
			<div class="panel-body">
				<div class="note note-info">
		        	<h4>Увага! Посилання для всіх товарів</h4>
		        	<p> <a href="<?=SERVER_URL.$_SESSION['alias']->alias?>/exportyml?key=<?=$_SESSION['option']->exportKey?>"><?=SERVER_URL.$_SESSION['alias']->alias?>/exportyml?key=<?=$_SESSION['option']->exportKey?></a> </p>
		        </div>
			    <form action="<?= SITE_URL?>admin/export" class="table-responsive" method="POST" >
								...
								if(!empty($list)){
									function showList($all, $list, $count_childs, $parent = 0, $level = 0)
									{
										...
										foreach ($list as $g) if($g->parent == $parent) { ?>
											<tr>
												<td><input type="checkbox" name="groups[]"  value="<?=$g->id?>"></td>
												...
												<td>
													<a href="<?=SERVER_URL.$_SESSION['alias']->alias?>/exportyml?key=<?=$_SESSION['option']->exportKey?>&group=<?=$g->id?>"><?=SERVER_URL.$_SESSION['alias']->alias?>/exportyml?key=<?=$_SESSION['option']->exportKey?>&group=<?=$g->id?></a>
												</td>
											</tr>
										<?php
											if(!empty($g->child)) {
			        ...
			        <div class="col-md-12 text-center">
				    	<!-- <input type="submit" name="excel" class="btn btn-success" value="Згенерувати Excel"> -->
				    	<input type="submit" name="yml" class="btn btn-success" value="Згенерувати YML">
				    </div>
			    </form>
			</div>
		</div>
