...
/*
 	...
 	Service "Shop Storage + price per amount 1.0"
	for WhiteLion 1.0

*/

class shopstorage_price_per_amount extends Controller {
	...
	public function __get_Invoices_to_Product($id = 0)
	{
		...
		if($productId > 0)
		{
			...
			return $this->storage_ppa_model->getProducts($productId, $userType);
		}
		return false;
	}
	...
	public function __get_Invoice($id = 0)
	{
		...
		{
			$this->load->smodel('storage_ppa_model');
			return $this->storage_ppa_model->getInvoice($invoiceId, $userType);
		}
		return false;
	}
	...
	{
		$this->load->smodel('storage_ppa_model');
		return $this->storage_ppa_model->setReserve($data);
	}
	...
	{
		$this->load->smodel('storage_ppa_model');
		return $this->storage_ppa_model->setBook($data);
	}
