...
class shopstorage_price_per_amount_admin extends Controller {
    ...
    {
    	$this->load->smodel('storage_ppa_model');
    	if(is_numeric($uri))
    	{
    		$product = $this->storage_ppa_model->getProduct($uri, -1);
    		if($product)
    		{
    			...
    			$product->history = $this->storage_ppa_model->getProducts($product->product, true);
    	...
    	else
    	{
    		...
    		if(isset($_GET['article']))
    		{
    			...
    			if($products) 
    			{
    				foreach ($products as $product) {
				    	$invoicesProduct = $this->storage_ppa_model->getProducts($product->id, -1);
				    	if($invoicesProduct)
				    	{
    		...
    		elseif(isset($_GET['id']))
    		{
    			...
    			if($product) 
    			{
			    	$invoices = $this->storage_ppa_model->getProducts($product->id, -1);
			    	if($invoices)
			    	{
    		...
    		{
    			$invoices = $this->storage_ppa_model->getProducts(0, -1);
		    	if($invoices)
		    	{
	...
	public function add()
	{
		...
		$this->load->smodel('storage_ppa_model');
		$storage = $this->storage_ppa_model->getStorage();
	...
	public function edit()
	{
    	...
    	if(is_numeric($id))
    	{
    		...
    		$this->load->smodel('storage_ppa_model');
    		$product = $this->storage_ppa_model->getProduct($id, -1);
    		if($product)
    		{
    			$product->info = $this->getProduct('id', $product->product);
    			$product->history = $this->storage_ppa_model->getProducts($product->product);
    			$storage = $this->storage_ppa_model->getStorage();
	...
	public function save()
	{
		...
		if(isset($_POST['id']) && is_numeric($_POST['id']))
		{
			...
			if($_POST['id'] == 0)
			{
				$id = $this->storage_ppa_model->save();
				if($id){
			...
			{
				if($this->storage_ppa_model->save($_POST['id']))
				{
					$id = $_POST['id'];
	...
	public function delete()
	{
		...
		{
			$this->load->smodel('storage_ppa_model');
			if($this->storage_ppa_model->delete($_POST['id']))
			{
				$_SESSION['notify'] = new stdClass();
