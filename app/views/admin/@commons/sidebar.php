...
<ul class="nav">
  <li class="nav-profile">
    <div class="image">
      <a href="<?=SITE_URL?>admin/wl_users/my"><img src="<?=SITE_URL?>style/admin/images/user-8.jpg" alt="" /></a>
    </div>
    <div class="info">
      <?=$_SESSION['user']->name?>
...
<ul class="nav">
  ...
  <?php
      $where['active'] = 1;
      $where['service'] = '>0';
      $this->db->select('wl_aliases', '*', $where);
      $this->db->join('wl_services', '', '#service');
      $this->db->order('order_alias DESC', 'wl_services');
      $wl_aliases = $this->db->get('array');
      if($wl_aliases){
        $sub_menus = array();
        $sub_menus_data = $this->db->getAllDataByFieldInArray('wl_options', array('alias' => '<0', 'name' => 'sub-menu'));
        if($sub_menus_data){
        ...
        foreach ($wl_aliases as $wl_alias) if($this->userCan($wl_alias->alias)) {
          $where = array();
              ...
              <ul class="sub-menu">
                ...
                foreach ($sub_menu as $sm) {
                  ...
                  if(isset($sb['alias']) && $sb['name']){
                    echo('<li');
                    if($this->data->uri(2) == $sb['alias']) echo(' class="active"');
